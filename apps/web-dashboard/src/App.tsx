...
const pages: Array<{ id: Page; icon: typeof Factory; en: string; zh: string; code: string }> = [
  { id: 'home', icon: Factory, en: 'Ops Console', zh: '运营总控', code: 'OPS-00' },
  ...
  { id: 'notice', icon: ClipboardList, en: 'Release Notice', zh: '放行通知', code: 'PN-04' },
  ...
  { id: 'qa', icon: Search, en: 'Ops Q&A', zh: '运营问答', code: 'QA-07' },
  { id: 'agent', icon: GitBranch, en: 'Tool Trace', zh: '工具轨迹', code: 'AGT-08' },
...
const loopLabels = ['ERP', 'WMS', 'Excel', 'Intelligence Layer', 'Dashboard', 'Simulation', 'Notice', 'Agent Tools'];
const flowLabels = ['File import', 'BOM demand', 'Inventory coverage', 'Release notice', '24h simulation', 'Tool trace'];
  ...
  return (
    <div className="page-grid home-grid">
      ...
      <section className="wide-panel">
        <h3>{t(lang, 'Operating Loop', '运营闭环')}</h3>
...
function ImportCenter({ lang }: { lang: Lang }) {
  return (
    <section>
      <SectionHeader code="IMP-01" title={t(lang, 'Data Import Center', '数据导入中心')} subtitle={t(lang, 'Classified demo files with parser and source trace.', '带解析器和来源追溯的演示文件分类。')} />
...
function TracePage({ lang }: { lang: Lang }) {
  return (
    <section>
      <SectionHeader code="TRC-03" title={t(lang, 'Product Material Trace', '成品物料追溯')} subtitle={t(lang, 'Product, order, BOM, stock, inbound and supplier context.', '连接产品、订单、BOM、库存、在途和供应商上下文。')} />
...
function Notice({ lang }: { lang: Lang }) {
  return (
    <section>
      <SectionHeader code="PN-04" title={t(lang, 'Release Notice Generator', '放行通知生成器')} subtitle={t(lang, 'Preview built from product, order, BOM and material gate.', '根据产品、订单、BOM 和物料门禁生成预览。')} />
...
function Simulation({ lang }: { lang: Lang }) {
  return (
    <section>
      <SectionHeader code="SIM-05" title={t(lang, 'Production Line Simulation', '产线节拍仿真')} subtitle={t(lang, 'Machine states, buffers, transport links and deterministic 24h output.', '设备状态、缓冲、转运连接与 24 小时确定性产出。')} />
...
function Report({ lang }: { lang: Lang }) {
  return (
    <section>
      <SectionHeader code="REP-06" title={t(lang, 'Simulation Report', '仿真报告')} subtitle={t(lang, 'Bottleneck, output, scrap, utilization, waiting and blocking.', '瓶颈、产出、报废、利用率、等待和堵塞。')} />
...
function QA({ lang }: { lang: Lang }) {
  return (
    <section>
      <SectionHeader code="QA-07" title={t(lang, 'Operations Q&A', '运营问答')} subtitle={t(lang, 'Questions are answered through registered operations tools.', '问题通过已注册的运营工具回答。')} />
...
function Integrations({ lang }: { lang: Lang }) {
  return (
    <section>
      <SectionHeader code="INT-09" title={t(lang, 'Integration Status', '接口状态')} subtitle={t(lang, 'Adapter mode and scope remain explicit.', '接口模式和范围保持显式。')} />
...
function ProjectNotes({ lang }: { lang: Lang }) {
  return (
    <section>
      <SectionHeader code="DOC-10" title={t(lang, 'Project Notes', '项目说明')} subtitle={t(lang, 'A generic manufacturing operations prototype built from reproducible demo workflows.', '基于可复现演示工作流构建的通用制造运营原型。')} />
      <div className="case-columns">
        ...
        <div>
          <h3>System</h3>
          <p>The operating loop connects imports, BOM demand, stock coverage, release notice, simulation report and agent trace.</p>
        </div>
        <div>
          <h3>Agent design</h3>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-mark"><Factory size={28} /><span>Ops Intelligence</span></div>
        <nav>
          {pages.map((item) => {
